...
import {
  ...
  FolderTree, Plus, CornerDownRight, Lock, Layers
} from 'lucide-react';
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-4 text-xs">
            ...
            <div className="flex items-start gap-2.5 p-3 bg-indigo-50/60 rounded-xl border border-indigo-100 text-[11px] text-indigo-900">
              ...
              <span>
                A hierarquia é <b>Grupo → Categoria → Subcategoria</b>. <b>Receita</b> e <b>Despesa</b> são os grupos
                originais do sistema e não podem ser alterados; os grupos que criar declaram a sua natureza
                (entra/sai) para que o fluxo de caixa e o IVA continuem certos.
              </span>
            </div>
                    ...
                    <div key={g.id} className="border border-slate-200 rounded-xl overflow-hidden">
                      <div className="flex items-center justify-between gap-2 px-4 py-2.5 bg-slate-50 border-b border-slate-200">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {g.is_system && (
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-slate-200 text-slate-600 flex items-center gap-1">
                          ...
                          {roots.map((c) => (
                            <div key={c.id} className="px-4 py-2.5">
                              ...
                              {(c.children || categories.filter((s) => s.parent_id === c.id)).map((sub) => (
                                <div key={sub.id} className="pl-4 pt-1.5 flex items-center gap-2 flex-wrap">
